...
cur = conn.cursor()
# cur.execute('CREATE TABLE company(company_id SERIAL PRIMARY KEY, company_name varchar(100) NOT NULL, company_link text)')
...
for comps in companies:
    ...
    cur.execute('INSERT INTO company(company_name) VALUES (%s)', comp["name"])
conn.commit()
cur.close()
conn.close()
